...
func (a *App) ParseFile(req ParseFileRequest) (response ParseFileResponse) {
	...
	defer func() {
		if r := recover(); r != nil {
			...
			response = ParseFileResponse{
				Success:      false,
				ErrorMessage: fmt.Sprintf("解析過程中發生嚴重錯誤，請檢查檔案格式"),
			}
		}
	}()
	...
	// T146: 路徑遍歷攻擊防護 - 驗證檔案路徑
	cleanPath, err := filepath.Abs(req.FilePath)
	if err != nil {
		a.log.Warn().
			Err(err).
			Str("path", req.FilePath).
			Msg("路徑解析失敗")
	...
	if !fileInfo.Mode().IsRegular() {
		...
		return ParseFileResponse{
			Success:      false,
			ErrorMessage: "只能讀取一般檔案，不支援目錄或特殊檔案",
		}
	}
...
func (a *App) SelectSaveLocation(defaultName string) (response SelectSaveLocationResponse) {
	// T150: Panic recovery
	defer func() {
		if r := recover(); r != nil {
			a.log.Error().
			...
			response = SelectSaveLocationResponse{
				Success:      false,
				ErrorMessage: "選擇儲存位置時發生錯誤",
			}
		}
	}()
...
func (a *App) ExportToExcel(req ExportToExcelRequest) (response ExportToExcelResponse) {
	// T150: Panic recovery
	defer func() {
		if r := recover(); r != nil {
			a.log.Error().
			...
			response = ExportToExcelResponse{
				Success:      false,
				ErrorMessage: "匯出過程中發生嚴重錯誤",
			}
		}
	}()
	...
	// T146: 路徑驗證 - 驗證儲存路徑
	cleanSavePath, err := filepath.Abs(req.SavePath)
	if err != nil {
		a.log.Warn().
			Err(err).
		...
		return ExportToExcelResponse{
			Success:      false,
			ErrorMessage: "無效的儲存路徑",
		}
	}

	// 確保路徑已正規化
	if cleanSavePath != filepath.Clean(req.SavePath) {
		a.log.Warn().
			Str("original", req.SavePath).
			Str("clean", cleanSavePath).
			Msg("偵測到可疑的儲存路徑")
	}

	req.SavePath = cleanSavePath
...
// T151-T152: 最近檔案列表 API

// GetRecentFilesResponse 取得最近檔案列表的回應
type GetRecentFilesResponse struct {
	Success bool         `json:"success"` // 是否成功
	Files   []RecentFile `json:"files"`   // 最近檔案列表
}

// RecentFile 最近檔案資訊（對應前端）
type RecentFile struct {
	Path       string `json:"path"`       // 檔案路徑
	Name       string `json:"name"`       // 檔案名稱
	Size       int64  `json:"size"`       // 檔案大小
	OpenedAt   string `json:"openedAt"`   // 開啟時間（ISO 8601）
	TotalLines int    `json:"totalLines"` // 總行數
}

// GetRecentFiles 取得最近開啟的檔案列表（T151）
func (a *App) GetRecentFiles() GetRecentFilesResponse {
	a.log.Info().Msg("取得最近檔案列表")

	recentFiles := a.state.GetRecentFiles()

	// 轉換為前端格式
	files := make([]RecentFile, len(recentFiles))
	for i, rf := range recentFiles {
		files[i] = RecentFile{
			Path:       rf.Path,
			Name:       rf.Name,
			Size:       rf.Size,
			OpenedAt:   rf.OpenedAt.Format(time.RFC3339),
			TotalLines: rf.TotalLines,
		}
	}

	a.log.Info().Int("count", len(files)).Msg("返回最近檔案列表")

	return GetRecentFilesResponse{
		Success: true,
		Files:   files,
	}
}

// ClearRecentFilesResponse 清空最近檔案列表的回應
type ClearRecentFilesResponse struct {
	Success      bool   `json:"success"`      // 是否成功
	ErrorMessage string `json:"errorMessage"` // 錯誤訊息
}

// ClearRecentFiles 清空最近開啟的檔案列表（T152）
func (a *App) ClearRecentFiles() ClearRecentFilesResponse {
	a.log.Info().Msg("清空最近檔案列表")

	if err := a.state.ClearRecentFiles(); err != nil {
		a.log.Error().Err(err).Msg("清空最近檔案列表失敗")
		return ClearRecentFilesResponse{
			Success:      false,
			ErrorMessage: fmt.Sprintf("清空失敗: %v", err),
		}
	}

	a.log.Info().Msg("最近檔案列表已清空")
